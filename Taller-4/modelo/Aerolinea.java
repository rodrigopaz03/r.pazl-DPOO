...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	int i = 0;
        while(i<n) {
        	if (vuelos.get(1).getRuta().getCodigoRuta().equals(codigoRuta) && vuelos.get(i).getFecha().equals(fechaVuelo)) {
        		return vuelos.get(i);
        	}
        	i++;
        }
    ...
    public void cargarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException, InformacionInconsistenteException
    {
        ...
        PersistenciaAerolineaJson persistencia = new PersistenciaAerolineaJson();
        persistencia.cargarAerolineas(archivo, this);
    }
    ...
    { 
    	Vuelo vuelo = null;
    	for (Vuelo v : vuelos) {
	        if (v.getRuta().getCodigoRuta().equals(codigoRuta) && v.getAvion().getNombre().equals(nombreAvion) && !v.getFecha().equals(fecha) ) {
	            vuelo = v; }
	    if (vuelo.equals(null)) {throw new Exception();}
        vuelos.add(vuelo);
    }}
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	 Cliente cliente = clientes.get(identificadorCliente);
    	    Vuelo vuelo = null; // Declaración fuera del bucle for
    	    for (Vuelo v : vuelos) {
    	        if (v.getRuta().getCodigoRuta().equals(codigoRuta)) {
    	            vuelo = v; 
    	            break; //Use break para poder salir del bucle
    	        }
    	    }

    	    if (vuelo == null) {
    	        throw new Exception("No se encontró el vuelo con el código de ruta especificado");
    	    }
    	    ...
    	    if ((mes >= 1 && mes <= 5) || (mes >= 9 && mes <= 11)) {
    	        ...
    	        if (vuelo.getAvion().getCapacidad() - cantidad > 0 ) {
    	        	precio = tarifa * cantidad;
    	        	int i = 1;
    	        	while (i<= cantidad) {
    ...
    {	int saldo = 0;
        Cliente cliente = null;
        	...
        	saldo = cliente.calcularValorTiquetes();
        	}
         return "El saldo pendiente es de "+saldo;
    }
